get_infiniWeapon_owner = {
    private ["_result"];
	_result = objNull;
	{
		if (infiniWeaponID in weapons _x)  then {
			_result = _x;
		}
	} forEach allUnits;
    _result
...
opfor_has_infiniWeapon = {
    private ["_result", "_currentOwner"];
	_result = false;
	_currentOwner = [] call get_infiniWeapon_owner;
...
opfor_has_won_in_trigger = {
	...
	_result = false;
	_trigger = _this;
	_launcher_owner = [] call get_infiniWeapon_owner;
